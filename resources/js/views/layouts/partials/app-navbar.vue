<template>
  ...
  >
    ...
    <a
      class="nav-link toggle-left-sidebar d-lg-none pr-4"
    ...
    <ul class="nav nav-inline d-none d-lg-flex mr-2">
      <li class="nav-item">
        <a
          class="nav-link toggle-left-sidebar"
          v-b-tooltip.hover
          :title="$t('config.ui.toggle_sidebar')"
          @click="toggleLeftSidebar"
          ><i class="fas fa-bars"></i
        ></a>
      </li>
      <li class="nav-item">
        <a
          class="nav-link toggle-full-screen"
          v-if="uiConfigs.fullScreen"
          v-b-tooltip.hover
          :title="$t('config.ui.toggle_fullscreen')"
    ...
    <ul class="nav nav-inline d-none d-md-flex mr-3">
      <li
        class="nav-item"
        v-if="configs.system && configs.system.maintenanceMode"
      >
        <span class="warning-label badge badge-warning">{{
          $t("config.system.under_maintenance")
        }}</span>
      </li>
      <li class="nav-item" v-if="configs.system && !configs.system.mode">
        <span class="warning-label badge badge-warning">{{
          $t("config.system.test_mode")
        }}</span>
      </li>
      <li class="nav-item ml-2" v-if="!isOnline">
        <span class="warning-label badge badge-error"
          ><i class="fas fa-exclamation-triangle"></i>
          {{ $t("general.offline") }}</span
        >
      </li>
    </ul>
    ...
    <ul class="navbar-nav">
      ...
      <!-- <li class="nav-item d-none d-md-flex align-items-center">
        <a
          class="nav-link cta bg-success text-white"
          href="https://codecanyon.net/item/kodeui-laravel-vuejs-bootstrap-spa-admin-starter-kit/27780969"
        >
          BUY IT NOW
        </a>
      </li> -->
      ...
      <!-- <base-dropdown
        ...
        v-if="
          configs.system &&
          configs.system.notifications &&
          hasPermission('access-notifications')
        "
      >
        <a
          slot="title"
      ...
      <!-- <base-dropdown
        ...
        position="right"
        v-if="
          configs.system && configs.system.todo && hasPermission('access-todo')
        "
      >
        <a
          slot="title"
      ...
      <!-- <li
        ...
        v-if="hasPermission('access-config')"
      >
        <router-link class="nav-link" :to="{ name: 'appConfig' }"
      ...
      <base-dropdown
        ...
        position="right"
        :open.sync="userDropddownOpen"
      >
        <a
          slot="title"
          class="nav-link nav-link-icon nav-user-avatar p-0"
          data-toggle="dropdown"
          role="button"
        >
          ...
          <user-avatar
            ...
            :user="loggedInUser"
            :size="40"
            :color="vars.colors[uiConfigs.topNavbarColor]"
          />
        </a>

        <h6 class="dropdown-header">
          {{ $t("general.greeting") + ", " + loggedInUser.name }}
        </h6>

        <router-link
          v-if="loggedInUser"
          :to="{ name: 'appProfileView' }"
          class="dropdown-item"
        >
        ...
        <a
          v-if="configs.auth.lockScreen"
          href="#"
          @click.prevent="lock"
          class="dropdown-item"
          ><i class="fas fa-lock"></i> {{ $t("auth.lock.lock") }}</a
        >

        <router-link
          :to="{ name: 'appUserChangePassword' }"
          class="dropdown-item"
        >
        ...
        <a href="#" @click.prevent="logout" class="dropdown-item"
          ><i class="fas fa-power-off"></i> {{ $t("auth.logout") }}</a
        >
      </base-dropdown>
    </ul>
  </nav>
...
export default {
  ...
  computed: {
    ...mapGetters("config", ["configs", "uiConfigs", "vars"]),
    ...mapGetters("user", {
      getUserDetails: "get",
      hasRole: "hasRole",
  ...
  methods: {
    ...mapActions("config", ["SetUiConfig"]),
    ...mapActions("user", ["Logout", "Lock"]),
    ...
    toggleSearchScreen() {
      this.$gaEvent(
        "engagement",
        "toggleSearchScreen",
        this.uiConfigs.searchScreenShow ? "hide" : "show"
      );
    ...
    takeScreenshot() {
      ...
      Vue.nextTick().then(() => {
        html2canvas(document.querySelector(".__view"), { logging: false }).then(
          (canvas) => {
