      ...
      >
        ...
        >
          <b-table
            ...
            :filters="null"
            @row-dblclicked="
              rowClickHandler({ route: 'appUtilityTodoView' }, $event)
            "
          >
            <template #cell(date)="row">
              <view-date
                v-if="row.item.time"
                :value="`${row.item.date} ${row.item.time}`"
                with-tz
                tag="span"
                class="mb-0"
              />
            ...
            <template #cell(status)="row">
              <div class="pointer" @click.stop="toggleTodo(row.item)">
                <badge v-if="row.item.status" rounded type="primary"
                  ><i class="far fa-check-circle"></i>
                  {{ $t("utility.todo.completed") }}</badge
                >
                <badge v-else rounded type="dark"
                  ><i class="far fa-circle"></i>
                  {{ $t("utility.todo.incomplete") }}</badge
                >
              </div>
            </template>
            ...
            <template #cell(actions)="row">
              <base-button
                ...
                :title="
                  $t('global.toggle', { attribute: $t('utility.todo.todo') })
                "
                @click.stop="toggleTodo(row.item)"
              >
              ...
              <table-row-actions>
                <router-link
                  ...
                  {{
                    $t("global.view", { attribute: $t("utility.todo.todo") })
                  }}</router-link
                >
                ...
                <router-link
                  ...
                  {{
                    $t("global.edit", { attribute: $t("utility.todo.todo") })
                  }}</router-link
                >
                ...
                <a
                  ...
                  {{
                    $t("global.delete", { attribute: $t("utility.todo.todo") })
                  }}</a
                >
              </table-row-actions>
            </template>
