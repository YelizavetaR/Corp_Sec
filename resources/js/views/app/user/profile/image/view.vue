    ...
    <div class="row" v-if="loggedInUser">
      <div class="col-12">
        <h4 class="text-center mb-5">
          {{ $t("global.upload", { attribute: $t("user.props.image") }) }} for
          {{ loggedInUser.name }}
        </h4>
      </div>
      ...
      <div class="col-12 col-md-4 offset-md-4 d-flex justify-content-center">
        <image-uploader
          ...
          :existing-image="loggedInUser.avatar"
          :placeholder="getUserImage(loggedInUser.gender, loggedInUser.age)"
          @uploaded="getInitialData"
          remove-from-server
          @removed="getInitialData"
          profile
        />
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...mapGetters("config", ["configs", "vars"]),
    ...mapGetters("user", ["loggedInUser"]),
  },
  methods: {
    ...mapActions("common", ["Init", "Custom"]),
