<template>
  <app-dashboard-header>
    <div class="header-body min-height-150px">
      <animated-loader
        :is-loading="isLoading && !(others.stats && others.stats.length)"
        :loader-color="vars.colors.white"
        ...
        <div class="col-12" v-for="alert in others.alerts">
          ...
          >
            ...
            <template v-if="alert.link">
              <router-link
                v-if="alert.link.route"
                :to="{ name: alert.link.route }"
                class="btn btn-lg alert-btn"
                >{{ alert.link.text }}</router-link
              >
              <a
                v-else-if="alert.link.href"
                :href="alert.link.href"
                :target="alert.link.target ? alert.link.target : '_self'"
                >{{ alert.link.text }}</a
              >
            </template>
