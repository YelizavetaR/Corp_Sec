<template>
  ...
  >
    ...
    <div
      class="item-list-wrapper flex-grow"
      :class="{ 'fixed-height': heightShouldBeFixed }"
    >
        ...
        <div class="min-height-200px">
          ...
          <ul class="item-list" v-if="others.todos && others.todos.length">
            <li v-for="item in others.todos" :key="item.uuid" class="item-item">
              ...
              >
                <div
                  :class="['item-type', todoIconColors[item.status]]"
                  v-b-tooltip.hover.d500
                  :title="
                    item.status
                      ? $t('utility.todo.completed')
                      : $t('utility.todo.incomplete')
                  "
                >
                ...
                <div class="item-details">
                  ...
                  <div class="item-schedule small text-muted">
                    <template v-if="item.time !== null && item.time">{{
                      [`${item.date} ${item.time}`, vars.serverDateTimeFormat]
                        | moment("calendar", null, {
                          sameElse: vars.defaultDateFormat,
                        })
                    }}</template>
                    <template v-else>{{
                      [item.date, vars.serverDateFormat]
                        | moment("calendar", null, {
                          sameElse: vars.defaultDateFormat,
                        })
                    }}</template>
          ...
          <div
            v-else-if="!isLoading"
            class="d-flex align-items-center justify-content-center min-height-200px"
          >
            <h5 class="text-muted">
