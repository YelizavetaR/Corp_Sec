<template>
  <li>
    ...
    <template
      v-if="
        !item.children || item.children.length === 0 || item.meta.hideChildren
      "
    >
      <a
      ...
      >
        <menu-item-content
          ...
          :menu-letter-icon="menuLetterIcon"
        ></menu-item-content>
      </a>
    ...
    <template v-else-if="item.meta.isHiddenNav && item.children.length">
      <template
        v-for="(navItem, k) in item.children"
        v-if="!(navItem.meta.isHiddenNav && navItem.meta.hideChildren)"
      >
        <menu-item
          :menu-id="`${menuId}-${navItem.name}-${k}`"
          :item="navItem"
    ...
    <template v-else>
      <a
        :class="['has-children', isCollapseOpen ? 'is-open' : '']"
      ...
      <b-collapse
        ...
        :accordion="
          parent
            ? `sub-menu-accordion-${parent.title.length}`
            : 'menu-accordion'
        "
      >
